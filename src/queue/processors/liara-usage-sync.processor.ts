...
import {
  LiaraManagementService,
  type LiaraLogEntry,
} from '../../modules/liara/liara-management.service';
...
export class LiaraUsageSyncProcessor {
  ...
  @Process('sync')
  async handleSync() {
    const users = await this.prisma.liaraApiKey.findMany({
      select: { userId: true, keyName: true },
    });
      ...
      await Promise.all(
        batch.map(async ({ userId, keyName }) => {
          try {
            ...
            failed++;
            this.logger.warn(
              `Liara usage sync failed for user=${userId}: ${(err as Error).message}`,
            );
          }
        }),
      );
    ...
    this.logger.log(
      `Liara usage sync: ${synced} user(s) synced, ${failed} failed, date=${date.toISOString().slice(0, 10)}`,
    );
  }

  private async syncOneUser(
    userId: string,
    keyName: string,
    date: Date,
    from: Date,
    ...
    for (const log of logs) {
      ...
      const tokens =
        (details?.tokens_prompt ?? 0) + (details?.tokens_completion ?? 0);
