...
export class AuthService {
  ...
  private getOtpRateLimit(): number {
    return Number(
      this.config.get('OTP_RATE_LIMIT', String(DEFAULT_OTP_RATE_LIMIT)),
    );
  }
  ...
  private getOtpRateWindow(): number {
    return Number(
      this.config.get(
        ...
        String(DEFAULT_OTP_RATE_WINDOW),
      ),
    );
  }
  ...
  private getOtpAttemptLimit(): number {
    return Number(
      this.config.get('OTP_ATTEMPT_LIMIT', String(DEFAULT_OTP_ATTEMPT_LIMIT)),
    );
  }
  ...
  private getOtpAttemptWindow(): number {
    return Number(
      this.config.get(
        ...
        String(DEFAULT_OTP_ATTEMPT_WINDOW),
      ),
    );
  }
  ...
  async sendOtp(rawPhone: string): Promise<{ message: string }> {
    ...
    if (sends > this.getOtpRateLimit()) {
      throw new HttpException(
        fa.auth.otpTooManyRequests(Math.ceil(rateWindow / 60)),
        429,
      );
    }
  ...
  isOtpViewerEnabled(): boolean {
    ...
    return (
      this.config.get<string>('OTP_ADMIN_VIEWER_ENABLED', 'true') === 'true'
    );
  }
  ...
  async verifyOtp(
    ...
    if (isTestPhone) {
      if (code !== TEST_OTP_CODE)
        throw new UnauthorizedException(fa.auth.otpInvalid);
    } else {
    ...
    if (deviceUuid) {
      await this.deviceTokens
        ...
        .catch((err) =>
          this.logger.error(
            `attachToUser failed for deviceUuid=${deviceUuid} user=${user.id}`,
            err,
          ),
        );
    }
    ...
    if (anonSessionId) {
      await this.anonMigration
        ...
        .catch((err) =>
          this.logger.error(
            `anon migration failed for anonSessionId=${anonSessionId} user=${user.id}`,
            err,
          ),
        );
    }
  ...
  private async grantSignupCredits(userId: string): Promise<number | null> {
    try {
      ...
      this.logger.error(
        `grantSignupCredits failed for user=${userId}`,
        err as Error,
      );
      return null;
    }
