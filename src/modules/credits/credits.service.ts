...
export class CreditsService {
  ...
  async listPackages() {
    ...
    return packages.map(p => ({
      ...p,
      priceToman: this.computePackagePrice(p.credits, p.discountPercent, config),
    }))
  }
  ...
  async quoteCustomPrice(credits: number): Promise<{ priceToman: number }> {
    ...
    const [config, customPkg] = await Promise.all([
      this.getConfig(),
      this.prisma.creditPackage.findFirst({ where: { isCustomAmount: true, isActive: true } }),
    ])
    const priceToman = this.computePackagePrice(safeCredits, customPkg?.discountPercent ?? 0, config)
    return { priceToman }
  }
  ...
  async purchasePackage(userId: string, dto: PurchaseCreditPackageDto) {
    ...
    if (pkg.isCustomAmount) {
      if (!dto.customCredits || dto.customCredits < pkg.credits) {
        throw new BadRequestException(fa.discovery.customAmountBelowMinimum(pkg.credits))
      }
      effectiveCredits = dto.customCredits
    }

    const amountToman = this.computePackagePrice(effectiveCredits, pkg.discountPercent, config)
    if (amountToman <= 0) throw new BadRequestException(fa.validation.numberPositive)
