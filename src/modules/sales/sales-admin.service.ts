...
import {
  SalesConfigService,
  type UpdatableSalesBotConfig,
} from './sales-config.service';
import {
  SalesKbService,
  type SalesKbEntryInput,
  type SalesKbEntryUpdateInput,
} from './sales-kb.service';
...
export class SalesAdminService {
  ...
  async exportChatSessionsKb(
    sessionId?: string,
  ): Promise<{ entries: SalesKbDraftEntry[] }> {
    if (sessionId) {
      const session = await this.prisma.salesChatSession.findUnique({
        where: { sessionId },
      });
      if (!session) throw new NotFoundException('سشن پیدا نشد');
      return {
        entries: pairTurnsToKbDrafts(
          session.sessionId,
    ...
    const sessions = await this.prisma.salesChatSession.findMany({
      orderBy: { lastMessageAt: 'desc' },
    });
    const entries = sessions.flatMap((s) =>
      pairTurnsToKbDrafts(
        s.sessionId,
        s.messages as { role: string; content: string }[],
      ),
    );
    return { entries };
  }
  ...
  async getAnalyticsOverview(
    from: string,
    to: string,
  ): Promise<SalesBotAnalyticsOverview> {
    ...
    const totals = rows.reduce(
      (acc, r) => ({
        ...
        embeddingCostUsdMicros:
          acc.embeddingCostUsdMicros + r.embeddingCostUsdMicros,
  ...
  async getAnalyticsTimeseries(
    from: string,
    to: string,
  ): Promise<SalesBotAnalyticsPoint[]> {
    const rows = await this.prisma.salesBotDailyUsage.findMany({
      where: { date: { gte: new Date(from), lte: new Date(to) } },
      orderBy: { date: 'asc' },
    });
  ...
  async sendLeadSms(id: string, message: string) {
    ...
    if (!lead) throw new NotFoundException('لید پیدا نشد');
    if (!lead.phone)
      throw new BadRequestException('این لید شماره موبایل ندارد');
    ...
    return this.prisma.leadProfile.update({
      ...
      data: {
        followUpStatus:
          lead.followUpStatus === 'NEW' ? 'CONTACTED' : lead.followUpStatus,
      },
    });
  }
}
