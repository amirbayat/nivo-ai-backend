...
export class SalesService {
  ...
  async chat(
    ...
    if (count > 30)
      throw new HttpException('تعداد درخواست بیش از حد مجاز است', 429);
    ...
    try {
      ...
      return {
        reply:
          'سرویس هوش مصنوعی در حال حاضر در دسترس نیست. چند دقیقه دیگر دوباره امتحان کن 🙏',
        isDone: false,
      };
    }

    const recommendedPlan = this.extractPlan(text);
    const cost = await this.pricingService.calcCost(
      inputTokens,
      outputTokens,
      botConfig.model,
    );
  ...
  async saveLead(dto: SaveLeadDto): Promise<{ id: string }> {
    const base = {
      ...
      ...(dto.chatHistory !== undefined && {
        chatHistory: dto.chatHistory as object[],
      }),
      ...(dto.recommendedPlan !== undefined && {
        recommendedPlan: dto.recommendedPlan,
      }),
      ...(dto.discountRequested !== undefined && {
        discountOffered: dto.discountRequested,
      }),
  ...
  private async maybeOfferDiscount(
    ...
    const lead = await this.prisma.leadProfile.findUnique({
      where: { sessionId },
    });
