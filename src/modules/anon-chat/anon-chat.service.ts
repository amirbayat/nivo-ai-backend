...
export class AnonChatService {
  ...
  async getStatus(context: AnonContext) {
    ...
    if (pastFree) {
      ...
      stage =
        todayCount >= config.dailyMessageLimitAfterFree ? 'blocked' : 'limited';
    }
    ...
    const message =
      stage === 'blocked'
        ...
        : stage === 'limited'
          ? config.limitedZoneMessage
          : config.signupBannerMessage;
    ...
    return {
      ...
      remainingFree: Math.max(
        0,
        config.freeMessageLimit - identity.lifetimeMessageCount,
      ),
      remainingToday: pastFree
        ? Math.max(0, config.dailyMessageLimitAfterFree - todayCount)
        : null,
  ...
  async createConversation(
    ...
    const config = await this.configService.getConfig();
    if (!config.enabled)
      throw new ForbiddenException('چت آزمایشی موقتاً غیرفعال است');
  ...
  async getConversation(context: AnonContext, conversationId: string) {
    ...
    if (conversation.sessionId !== context.session.id)
  ...
  async streamChat(
    ...
    if (pastFree) {
      ...
      if (todayCount === 0) {
        this.funnelEvents
          .emit(session.id, 'ENTERED_LIMITED_ZONE')
          .catch(() => {});
      }
    }
    ...
    if (estimatedInput > config.maxInputTokens) {
      throw new BadRequestException(
        `پیام شما بیش از حد مجاز طولانی است (حداکثر ${config.maxInputTokens} توکن).`,
      );
    }

    const existingMessageCount = await this.prisma.anonymousMessage.count({
    ...
    try {
      const result = streamText({
        ...
        ...(config.reasoningEffort
          ? { reasoning: config.reasoningEffort as ReasoningEffortValue }
          : {}),
        onError: ({ error }) => {
      ...
      res.write(
        `data: ${JSON.stringify({
          error: isModelError
            ? 'مدل در دسترس نیست، لطفاً دوباره امتحان کنید'
