...
export class LiaraManagementService {
  ...
  private baseUrl(): string {
    return (
      this.config.get<string>('LIARA_MANAGEMENT_BASE_URL') ||
      'https://ai.liara.ir'
    );
  }
  ...
  private requireCredentials(): { jwt: string; workspaceName: string } {
    ...
    if (!jwt || !workspaceName) {
      throw new Error(
        'Liara management credentials (LIARA_MANAGEMENT_JWT/LIARA_WORKSPACE_NAME) not configured',
      );
    }
  ...
  async createApiKeyForUser(
    keyName: string,
  ): Promise<{ key: string; liaraKeyId: string }> {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Liara create-api-key failed (${res.status}): ${text.slice(0, 300)}`,
      );
    }
    const json = (await res.json()) as { key?: string; _id?: string };
    if (!json.key || !json._id)
      throw new Error('Liara create-api-key returned no key/_id');
  ...
  private async resolveWorkspaceId(
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => '');
      throw new Error(
        `Liara list-keys failed (${res.status}): ${text.slice(0, 300)}`,
      );
    }
    ...
    const workspace = json
      .flatMap((k) => k.workspaces ?? [])
      .find((w) => w.name === workspaceName);
    if (!workspace?._id)
      throw new Error(
        `Liara workspace "${workspaceName}" not found among existing keys`,
      );
  ...
  async fetchUsageLogs(
    ...
    for (;;) {
      ...
      const res = await fetch(
        `${this.baseUrl()}/v1/workspaces/${workspaceId}/logs?${params.toString()}`,
        {
          headers: { Authorization: `Bearer ${jwt}` },
          signal: AbortSignal.timeout(20_000),
        },
      ...
      if (!res.ok) {
        const text = await res.text().catch(() => '');
        throw new Error(
          `Liara list-logs failed (${res.status}): ${text.slice(0, 300)}`,
        );
      }
