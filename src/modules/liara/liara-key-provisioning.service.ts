...
export class LiaraKeyProvisioningService {
  ...
  async getApiKeyForUser(userId: string): Promise<string> {
    try {
      const existing = await this.prisma.liaraApiKey.findUnique({
        where: { userId },
      });
      if (existing) {
      ...
      try {
        ...
        if (
          ...
          const winner = await this.prisma.liaraApiKey.findUnique({
            where: { userId },
          });
          if (winner) {
            const winnerKey = decryptSecret(
              winner.encryptedKey,
              this.encryptionSecret(),
            );
            await this.clearIssue(userId);
  ...
  private async recordIssue(userId: string, message: string): Promise<void> {
    try {
      ...
      this.logger.warn(
        `Failed to record provisioning issue for user=${userId}: ${(err as Error).message}`,
      );
    }
  }
  ...
  private async clearIssue(userId: string): Promise<void> {
    try {
      await this.prisma.liaraKeyProvisioningIssue.deleteMany({
        where: { userId },
      });
    } catch (err) {
      this.logger.warn(
        `Failed to clear provisioning issue for user=${userId}: ${(err as Error).message}`,
      );
    }
  }
