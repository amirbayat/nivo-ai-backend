...
export class PushNotificationsService {
  ...
  async send(adminId: string, dto: SendPushNotificationDto) {
    const normalizedPhoneList =
      dto.segment === PushCampaignSegment.PHONE_LIST
        ? (dto.phoneList ?? []).map(normalizePhone)
        : [];
    const planIds =
      dto.segment === PushCampaignSegment.BY_PLAN ? (dto.planIds ?? []) : [];
    ...
    const { sentCount, failedCount, invalidTokens } =
  ...
  private async resolveTokens(
    ...
    if (segment === PushCampaignSegment.PHONE_LIST && !phoneList.length)
      return [];
