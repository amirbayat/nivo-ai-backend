...
export class CampaignService {
  ...
  async getActiveCampaign(): Promise<LaunchCampaign | null> {
    ...
    if (cached)
      return cached === 'null'
        ? null
        : this.reviveCampaignDates(JSON.parse(cached));

    const campaign = await this.prisma.launchCampaign.findFirst({
      where: { status: 'ACTIVE' },
    });
   ...
   */
  async applyToNewUser(
    userId: string,
    phone: string,
  ): Promise<{ message: string; queuePosition: number } | null> {
    ...
    const waitlistIsFull =
      campaign.maxWaitlistSize !== null &&
      waitingCount >= campaign.maxWaitlistSize;
    ...
    const message =
      waitlistIsFull && campaign.waitlistFullMessage
        ? campaign.waitlistFullMessage
        : campaign.waitlistMessage;
  ...
  async getWaitingDailyLimit(userId: string): Promise<number | null> {
    ...
    const limit =
      entry && entry.status === 'WAITING'
        ? entry.campaign.waitlistDailyMessageLimit
        : null;
    await this.redis.set(
      cacheKey,
      limit === null ? 'null' : String(limit),
      'EX',
      WAITING_LIMIT_CACHE_TTL,
    );
    return limit;
  }
  ...
  async getCampaignStatus(): Promise<CampaignStatus> {
    ...
    const initialPct = randomIntInclusive(
      campaign.displayInitialPctMin,
      campaign.displayInitialPctMax,
    );
    const floor = randomIntInclusive(
      campaign.displayFloorMin,
      campaign.displayFloorMax,
    );
    const initial = Math.max(
      Math.ceil((campaign.capacity * initialPct) / 100),
      floor + 1,
    );
  ...
  async getMyWaitlistStatus(
    userId: string,
  ): Promise<{ status: string; queuePosition: number | null } | null> {
    const entry = await this.prisma.waitlistEntry.findUnique({
      where: { userId },
    });
  ...
  private async grantEntries(entries: WaitlistEntry[]): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (campaign?.grantedSmsTemplate) {
        ...
        this.sms
          .sendByTemplate(entry.phone, campaign.grantedSmsTemplate, {
            token: link,
          })
          .catch(() => {});
      }
  ...
  async sendDueReminders(): Promise<void> {
    ...
    for (const entry of granted) {
      const steps = (entry.campaign.reminderSteps as ReminderStep[]).sort(
        (a, b) => a.dayOffset - b.dayOffset,
      );
      if (!steps.length) continue;

      const daysSinceGrant = Math.floor(
        (Date.now() - entry.grantedAt!.getTime()) / 86_400_000,
      );
      ...
      await this.sms
        .sendByTemplate(entry.phone, nextStep.template)
        .catch(() => {});
      await this.prisma.waitlistEntry.update({
        where: { id: entry.id },
        data: {
          lastReminderStepSent: nextStepIndex,
  ...
  async listCampaigns() {
    return this.prisma.launchCampaign.findMany({
  ...
  async createCampaign(data: {
    ...
    const existingActive = await this.prisma.launchCampaign.findFirst({
      where: { status: 'ACTIVE' },
    });
    if (existingActive) {
