...
export class DiscoveryController {
  ...
  @Get('images/:key')
  async getImage(@CurrentUser() user: JwtPayload, @Param('key') key: string, @Res() res: Response) {
    const { buffer, mimeType } = await this.discoveryService.getImage(user.sub, key)
  ...
  @Post('requests')
  createRequest(@CurrentUser() user: JwtPayload, @Body() dto: CreatePromptRequestDto) {
