...
export class DiscoveryAnonService {
  ...
  ): Promise<{ available: boolean; usedAt: string | null }> {
    return {
      available: identity.discoveryTrialUsedAt === null,
      usedAt: identity.discoveryTrialUsedAt?.toISOString() ?? null,
    };
  }
  ...
  async generate(context: AnonContext, dto: GenerateAnonCreativeDto) {
    ...
    if (claim.count === 0) {
      throw new HttpException(
        {
          message: fa.discovery.anonTrialAlreadyUsed,
          code: 'discovery_trial_used',
        },
        403,
      );
    }
    ...
    try {
      ...
      await this.prisma.anonymousIdentity.update({
        ...
        data: { discoveryTrialUsedAt: null },
      });
      throw err;
    }
  }
