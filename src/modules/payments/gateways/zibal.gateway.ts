...
export class ZibalGateway implements PaymentGateway {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    this.merchantId = isTestMode
      ? 'zibal'
      : this.config.get<string>('ZIBAL_MERCHANT_ID')!;
  }
  ...
  async createPayment({
    ...
    this.logger.log(
      `request → ${JSON.stringify({ ...body, merchant: maskSecret(body.merchant) })}`,
    );
    ...
    this.logger.log(
      `request ← status=${res.status} body=${JSON.stringify(json)}`,
    );
  ...
  async verifyPayment({
    ...
    this.logger.log(
      `verify → ${JSON.stringify({ ...body, merchant: maskSecret(body.merchant) })}`,
    );
    ...
    this.logger.log(
      `verify ← status=${res.status} body=${JSON.stringify(json)}`,
    );
  ...
  parseCallback(query: Record<string, string>): CallbackQuery {
    ...
    return {
      providerRef: query.trackId,
      success: query.success === '1' || query.success === '2',
    };
  }
}
